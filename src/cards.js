...
export const FUNNY={
  ...
  'Byt Hand':'"Byt hand. Kaos börjar."','Byt Färg':'"Välj färg. Makt!"','Dra 4':'"Näste drar 4. Förlåt inte."',
}

// playerCount: om 2 spelare, skippa Frys-kortet
export function buildDeck(playerCount=6){
  ...
  const s2=['Hoppa Över','Vänd','Dra 2']
  // Frys ingår inte vid 2 spelare
  const s1=playerCount<=2?['Ge Bort','Stjäl','Byt Hand']:['Frys','Ge Bort','Stjäl','Byt Hand']
  for(let n=1;n<=9;n++){
    const c=NUM_COLORS[n]
    s2.forEach(s=>{cards.push(mk({type:'special',name:s,color:c.name,bg:c.bg}));cards.push(mk({type:'special',name:s,color:c.name,bg:c.bg}))})
    s1.forEach(s=>cards.push(mk({type:'special',name:s,color:c.name,bg:c.bg})))
  }
